...
/*
 * Modified by the mssql_native project on 2026-09-05.
 *
 * Change:
 *   Extended bcp_options() to accept a comma-separated list of
 *   SQL Server BCP hints instead of retaining only one hint.
 *
 * This modification remains licensed under the same terms as this file.
 */
...
bcp_options(DBPROCESS * dbproc, int option, BYTE * value, int valuelen)
{
	...
	case BCPHINTS:
		...
		/*
		 * Microsoft accepts a comma-separated hint list. Keep FreeTDS's
		 * historical single-hint behavior, but validate and retain the full
		 * list when callers combine the supported flag-style hints.
		 */
		if (memchr(value, ',', valuelen)) {
			int start = 0;
			while (start < valuelen) {
				int end, matched = 0;
				while (start < valuelen && (value[start] == ' ' || value[start] == '\t'))
					++start;
				end = start;
				while (end < valuelen && value[end] != ',')
					++end;
				...
				if (end == start)
					return FAIL;
				for (i = 3; hints[i]; ++i) {
					if ((int) strlen(hints[i]) == end - start &&
					    strncasecmp((char *) value + start, hints[i], end - start) == 0) {
						matched = 1;
						break;
					}
				}
				if (!matched)
					return FAIL;
				start = end;
				while (start < valuelen && value[start] != ',')
					++start;
				if (start < valuelen)
					++start;
			}
